...
BAKKESMOD_PLUGIN(DodgeOverlayPlugin, PluginName, "0.0.3", 0)
...
void DodgeOverlayPlugin::RenderImGui() {
  ...
  if (Begin(GetMenuTitle().c_str(), &isWindowOpen, windowFlags)) {
    ...
    ImColor color = (std::fabs(stickLocation.x) + std::fabs(stickLocation.y) < tempDodgeDeadzone) ? ImColor(1.0f, 0.0f, 0.0f, dodgeDeadzoneCrossedAlpha) : ImColor(0.0f, 1.0f, 0.0f, dodgeDeadzoneCrossedAlpha);
